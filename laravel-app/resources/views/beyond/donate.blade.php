...
@section('meta_description', 'Donate to Catholic Women\'s Association Cameroon by MTN or Orange Money.')
...
<div class="min-h-[70vh] bg-slate-50 py-10 px-4">
    <div class="max-w-md mx-auto bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
        <h1 class="text-2xl font-extrabold text-brand-blue text-center">Donate</h1>
        ...
        <form method="POST" action="{{ route('beyond.donate.store') }}" class="mt-6 space-y-5" x-data="{ amount: {{ (int) old('amount', 5000) }}, custom: false }">
            @csrf
            <div>
                <label class="text-sm font-semibold text-gray-700">Amount (XAF)</label>
                ...
                <input x-show="custom" x-cloak type="number" min="100" step="100" x-model.number="amount"
                       class="mt-3 w-full rounded-md border border-gray-200 px-3 py-2" placeholder="Amount">
                <input type="hidden" name="amount" :value="amount">
            </div>

            <div>
                <label class="text-sm font-semibold text-gray-700">MoMo number</label>
                <div class="mt-1 flex">
